...
Undra is built using an AI-native architecture. The interactive assistant layer is powered by the **Gemini 2.5 Flash** model via the official **Google GenAI SDK** (`google-genai` Python library).

#### Key Integration Architecture:
- **Fast, Multimodal Reasoning**: We utilize `gemini-2.5-flash` to process both text queries and image uploads. For example, when a user uploads a photo of a laundry booking panel or a recycling label, Gemini's advanced vision capabilities allow it to analyze the Swedish text in the image and explain it within the specific cultural and functional context of Lund student life.
...
- **The "Route" UX**: When a guardrail is triggered (either pre- or post-generation), the application bypasses standard chat output and returns a structured refusal card. This card:
  1. Gracefully explains that Undra is legally and safely prohibited from advising on this highly sensitive topic.
...
The development, hosting, and operations of Undra have been kept highly cost-efficient by leveraging the free tier of the Google GenAI API for rutin/triage loops, and utilizing Cloud Run trial credits. 

The total out-of-pocket expenditure to date is approximately **$5.00 USD**, representing actual prepaid Gemini API usage for our deployed application's Pro planning calls and multimodal reasoning.

The following table provides a detailed breakdown of the project's financial cost structure:

| Category | Description | Cost (USD) | Funding Source |
|---|---|---|---|
| **Domain Registration** | `undra.nu` registered via Swedish registrar Hostup | ~$10.00 USD (100 SEK) | Human Personal Out-of-Pocket |
| **GCP Cloud Run Hosting** | Web application hosting, container building, egress, and secret management | $0.00 USD | GCP $300 Free Trial Credit (Project `undra-504613`) |
| **Gemini API Usage (Paid)** | High-accuracy `gemini-2.5-flash` multimodal queries and Pro planning calls | ~$5.00 USD | Prepaid Balance (Project `undra-504613`) |
| **Gemini API Usage (Free)** | Standard routing and operator triage checks | $0.00 USD | Google Developer Free Tier (Project `undra-free`) |
| **Local Operations Box** | Local lab development box (`red` running Ubuntu Server) electricity | ~$3.00 USD (30 SEK) | Personal Household Utility |
| **Total Out-of-Pocket Spend** | **Domain registration and paid-tier Gemini API credits consumed** | **~$15.00 USD total** | **Human Personal Funding** |

---

### 2.2 Arms-Length Revenue Breakdown (May – August 2026)
Undra did not generate any arms-length revenue within the hackathon window. The monthly breakdown is detailed below:

| Month | Arms-Length Revenue (USD) | Explanation |
|---|---|---|
| **May 2026** | $0.00 | Product development had not yet commenced. |
| **June 2026** | $0.00 | Product development had not yet commenced. |
| **July 2026** | $0.00 | Local infrastructure setup and operational charter design phase. |
| **August 2026** | $0.00 | **Official MVP Launch**. Service launched in August 2026 ahead of Arrival Day. |
...
*   **Visual (Action)**: The screen shows Undra's in-memory processing (metadata is stripped instantly for privacy), and Gemini 2.5 Flash processes the query. Within seconds, a response appears with the `🤖 [AI-Generated Response by Undra Assistant]` tag, giving a perfect English translation of the panel buttons and step-by-step instructions.
*   **Narrator (Voiceover)**:
    > "But what happens when you encounter a physical sign or a notice you can't read? Undra is powered by Gemini 2.5 Flash via the Google GenAI SDK. Students can upload photos of local signage, Swedish recycling bins, or appliance screens. Undra strips the EXIF metadata in-memory for absolute privacy, analyzes the image, and provides context-rich English answers in real-time."
